...
### 어려웠던 점
 ...
 <p>시간 초과가 자꾸 떠서 무엇이 처음에는 `append`, `pop`이 많아서 그런건가 생각이 들어 push, pop을 top을 이용하는 것으로 바꾸었다. 그래도 시간 초과가 떠서 질문방에 가보니 이 문제 자체가 input 과정이 많아서 `input` 함수로 인풋값을 받으면 시간초과가 뜬다고 하였다. 이를 보고 `input` 대신 `sys.stadin.readline()`을 사용하니 시간초과 문제가 해결됐다.</p>
